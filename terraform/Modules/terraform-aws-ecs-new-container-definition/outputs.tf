...
locals {
  json_raw = null_resource.test.*.triggers
  json_raw1 = replace(replace(replace(jsonencode(local.json_raw), "/(\\[\\]|\\[\"\"\\]|\"\"|{}|\"\\[\\]\"|\"{}\"|\"null\")/", "null"), "/\"(true|false)\"/", "$1"), "/\"(-?[0-9]+\\.?[0-9]*)\"/", "$1")
  json = replace(replace(replace(local.json_raw1, "/\"\\[(.*?)\\]\"/", "[$1]"), "/\\\\\"(.*?)\\\\\"/", "\"$1\""), "/\"{(.*?)}\"/", "{$1}")

  #encoded_environment_variables = length(local.environment) > 0 ? jsonencode(local.environment) : "null"
...
output "portmapping" {
  description = "portmapping for the main container"
  value = element(var.port_mappings, 0)
}
